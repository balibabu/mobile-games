    ...
    return (
        <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
            ...
            <View style={styles.container}>
                ...
                {/* Note the wrapper container with fixed width constraints for the board layout */}
                                ...
                                {row.map((cell) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={[styles.cell, { backgroundColor: isRevealed ? '#09090b' : '#18181b' }]}
                                            onPress={() => handleCellPress(cell.row, cell.col)}
                                            onLongPress={() => handleLongPress(cell.row, cell.col)}
                                            activeOpacity={0.2}
                                            delayPressOut={false}
                                        >
                                            {/* The wrapper handles absolute centering without altering container layout */}
                                            <View style={styles.cellContentWrapper}>
                                                {isFlagged && !isRevealed && <Flag size={14} color="#ef4444" strokeWidth={2.5} />}
                                                {isRevealed && isMine && <Bomb size={14} color="#ef4444" strokeWidth={2.5} />}
                                                {isRevealed && !isMine && cell.neighborMines > 0 && (
                                                    <Text style={[styles.cellNumber, { color: CELL_COLORS[cell.neighborMines] || '#f4f4f5' }]}>
                                                        {cell.neighborMines}
                                                    </Text>
                                                )}
                                            </View>
...
const styles = StyleSheet.create({
    ...
    // UPDATED: Added flex: 1 to expand and dynamically center its child (the game board)
    boardContainer: {
        width: '100%',        
        flex: 1,                // Takes up all remaining vertical space
        alignItems: 'center',
        justifyContent: 'center', // Centers the game board vertically within that space
        paddingVertical: 12,
    },
    gameBoard: { 
        width: '100%',        
